...
}> = ({ showSetting, isToken1InputFirst, handleSwap, onSubmit, isLoading, token1, token2, counttok1,
    setToken1, setToken2, setCountTok1, desiredPrice, setDesiredPrice, setUrlRpcX, setCommissionX, setSlippageX }) => {
        ...
        //Обработчик изменения поля counttok1
        const handleCountDesiredPrice = (e: React.ChangeEvent<HTMLInputElement>) => {
            const value = e.target.value;
            if (/^[\d.,]*$/.test(value) || value === '') {
                    ...
                    {isToken1InputFirst ? (
                        <>
                            ...
                            <input
                                ...
                                name="limitprice"
                                value={desiredPrice}
                                onChange={handleCountDesiredPrice}
                                placeholder="Введите цену"
                                required
                                className="w-full py-3 px-3 border rounded-md focus:outline-none focus:border-blue-500 bg-gray-700 text-white"
                            />
                        </>
                    ) : (
                        <>
