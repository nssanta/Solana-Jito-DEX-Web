...
import SwapFormLimit from '../components/SwapFormLimit';
import SwapResultLimit from '../components/SwapResultLimit';
import useSwapLimit from '../hooks/useSwapLimit';
...
const LimitSwap: React.FC = () => {
    ...
    const [desiredPrice, setDesiredPrice] = useState(''); // Новое состояние для "желаемой цены"
    ...
    const { onSubmit } = useSwapLimit(
        ...
        token2,
        counttok1,
        desiredPrice, // Передаем desiredPrice в функцию useSwap
        urlRpcRef,
        commissionRef,
        slippageRef
        ...
        <div className="w-full max-w-sm mx-auto p-2 sm:p-4 md:p-6 lg:p-8 bg-gray-800 rounded-md shadow-md form-container overflow-hidden">
            <h2 className="font-sans text-center text-2xl font-semibold text-white mb-6">#НАШ SWAP</h2>
                ...
                <SwapFormLimit
                    ...
                    desiredPrice={desiredPrice}  // значение цены лимита.
                    setDesiredPrice={setDesiredPrice} // сетер изменения лимита.
                    setUrlRpcX={setUrlRpc}
                    setCommissionX={setCommission}
                    setSlippageX={setSlippage}
                />
                <SwapResultLimit swapResult={swapResult} />
            </div>
        </div>
    ) : null;
};
